...
**D11 — Segurança aberta (resolvido).**
A configuração anterior liberava globalmente as requisições e mantinha CSRF desabilitado. A integração de segurança substituiu esse limite por uma allowlist pública explícita e default-deny em `SecurityConfigDefaultDeny`.
